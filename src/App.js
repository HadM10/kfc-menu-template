          ...
          <section key={category.name} className="mb-8">
            ...
            <div className="flex flex-wrap justify-center">
              {category.items.map((item) => (
                <div
                  className="bg-white border border-gray-300 rounded-lg shadow-md m-2 p-4 w-full sm:w-60 md:w-72 lg:w-80 flex flex-col" // Responsive widths
                  key={item.id}
                >
                  ...
                  <p className="font-bold text-lg text-right">{item.price}</p>
                </div>
              ))}
            </div>
          </section>
